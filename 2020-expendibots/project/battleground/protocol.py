"""
Helper module to implement either side of the Battle-Oriented Online Match
protocol (the 'BOOM' protocol).

This module provides a convenient Connection class to manage a BOOM connection
and MessageType flag enum to easily work with the various BOOM message types.

Example usage:

    >>> from protocol import Connection, MessageType as M
    >>> server = Connection.from_address('ai.far.in.net', 54321)
    >>> server.send(M.PLAY, name="bob_omb", channel="nova")
    >>> server.recv(M.OKAY)
...
# what server port should those using this protocol meet at?
DEFAULT_SERVER_PORT = 54321 # boom
